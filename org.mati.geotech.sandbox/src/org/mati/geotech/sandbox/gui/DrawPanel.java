...
public abstract class DrawPanel extends JPanel implements ComponentListener {
    ...
    private void init() {
        ...
        addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseMoved(MouseEvent e) {
                if (getModel() != null) {
                    getModel().getDisplayRect().setX(
                            scrToMapX(e.getPoint().x)
                                    - getModel().getDisplayRect().getWidth()
                                    / 2);
                    getModel().getDisplayRect().setY(
                            scrToMapY(e.getPoint().y)
                                    - getModel().getDisplayRect().getHeight()
                                    / 2);
                    repaint();
                }
    ...
    public void paintRect(Graphics g, Rect r) {
        if (r != null)
            g.drawRect((int) Math.round(mapToScrX(r.getX())), (int) Math
                    .round(mapToScrY(r.getY())), (int) Math.round(r.getWidth()
                    * _xsc), (int) Math.round(r.getHeight() * _ysc));
    }
