...
public class ObjectCatalog {
	// TODO: Loading objects catalog from remote database
	...
	public boolean isTypeFiltred(int type) {
		...
		for(String title: getCategoris()) {
			if(getObjectProps(title, name)!=null && 
					Boolean.parseBoolean(getObjectProps(title, name).get("filtred"))) {
				return true;							
			}
		}
		return false;
