...
public abstract class DrawPanel extends JPanel implements ComponentListener {
	...
	public double scrToMapX(double x) { return (x/(_xsc)+_xsh); }
	public double scrToMapY(double y) {	return (y/(_ysc)+_ysh); }

	public double mapToScrX(double x) { return (x-_xsh)*(_xsc); }
	public double mapToScrY(double y) {	return (y-_ysh)*(_ysc); }
	...
	private void init() {
		...
		addMouseMotionListener(new MouseMotionListener() {
			...
			public void mouseMoved(MouseEvent e) {
				if(getModel()!=null) {
					getModel().getDisplayRect().setX(scrToMapX(e.getPoint().x)
							- getModel().getDisplayRect().getWidth()/2 );
					getModel().getDisplayRect().setY(scrToMapY(e.getPoint().y)
							- getModel().getDisplayRect().getHeight()/2 );
					repaint();
				}
	...
	public void paintRect(Graphics g, Rect r) {
		if(r!=null)
		g.drawRect(
				(int)Math.round(mapToScrX(r.getX())),
				(int)Math.round(mapToScrY(r.getY())),
				(int)Math.round(r.getWidth()*_xsc),
				(int)Math.round(r.getHeight()*_ysc));
	}
	...
	public RectViewModel getModel() {return _model;}
	...
	public double getScreenWidth() { return _screenW; }
	public void setScreenWidth(double screenW) { this._screenW = screenW; }
	public double getScreenHeight() { return _screenH; }
	public void setScreenHeight(double screenH) { this._screenH = screenH; }	
}